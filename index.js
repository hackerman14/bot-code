...
client.once('ready',() => {
    ...
    client.user.setActivity('rh!help', { type: 'PLAYING' })
...
client.on('message', message =>{
    ...
    if(message.content.startsWith(`<@!609669181700571146>`)) {
        message.channel.send("yo what's up buddy")
    }
    ...
    if(message.content.includes("siri")) {
        message.channel.send("dude im not siri")
    }

    if(message.content.includes("alexa")) {
        message.channel.send("dude im not alexa")
    }
    ...
    if(message.content.startsWith(`${prefix}annoy`)) {
        ...
        if(!mentionedUser) return message.reply('mention someone to continue annoying someone!');
